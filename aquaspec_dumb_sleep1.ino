/*
  AquaSpec ESP32 + SPI OLED + Supabase (no BLE) — with CALIBRATED sensors
...
Reading takeReading();               // now uses your calibration flow
float measurePH();                   // calibrated
int   measureTDS(float TEMP);        // calibrated + temp compensation
...
// TDS: temp compensation + cubic -> ppm, then scale by 0.544 (your calibration)
...
float measureTemp() {
  ...
  // simple guard in case sensor missing
  if (temp < -100.0f || temp > 85.0f) temp = 25.0f;
  return temp;
}
...
Reading takeReading(){
  ...
  // interval per your snippet: 1000/NUM_SAMPLES (~100ms)
  unsigned long interval = 1000 / NUM_SAMPLES;

  // 1) Temperature first (so TDS can use compensated temp; also time for pH to normalize)
  ...
  // 2) pH + TDS (with temp compensation)
  for (int i = 0; i < NUM_SAMPLES; i++) {
...
// ================== ESP Sleep and Screen Power Off ==================

void turnOffOLED() { // Turns screen off
...
void goToSleep() { //Function to send device to sleep
  Serial.println("Going to sleep...");

  turnOffOLED();

  // Configure wakeup from SELECT button (active LOW)
...
const uint32_t BACK_HOLD_TIME_MS = 5000;       // 5 seconds hold
...
void setup() {
  ...
  delay(300);
  Serial.println();
  Serial.println("=== AquaSpec (SPI OLED + Supabase, calibrated) ===");
  ...
  if (!display.begin(SSD1306_SWITCHCAPVCC)) {
    ...
    display.setRotation(3); // same orientation you used
    drawSplash();
  }
...
void loop() {
  ...
  // Check for BACK button hold
  if (digitalRead(BACK_BUTTON) == LOW) {
    if (backPressStart == 0) {
      backPressStart = millis(); // just pressed
    } else if (millis() - backPressStart > BACK_HOLD_TIME_MS) {
      goToSleep();
    }
  } else {
    backPressStart = 0; // reset if released
  }
